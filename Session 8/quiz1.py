...
def leap_year(year):
    ...
    """
    if year % 4 != 0 or year % 100 == 0 and year > 1582 and year % 400 != 0: ## %module 4 to see if not leap year and greater than 1582 because after that date
	    return False
    else:
	    return True
...
def sum_squares(n):
    result = 0 #store result here 
    for x in range(n + 1): #plus one makes it inclusive
	    result += x ** 2
    return result
...
""""print(1+4+9+16+25+36+49+64+81+100) #ignore this doing comparing math """"